package com.cmcm.cmimexcemple.adapter;
...
public class MsgAdapter extends RecyclerView.Adapter<MsgAdapter.ReceiveViewHolder> {
     ...
     * 展示列表中所有消息
     *
     * @param localMsgs 消息集合
     */
    public void setMsgList(List<IMMsg> localMsgs) {
        msgList.clear();
        msgList.addAll(localMsgs);
        notifyDataSetChanged();
    }
    ...
    public void onBindViewHolder(@NonNull ReceiveViewHolder holder, int position) {
        ...
        if (TextUtils.isEmpty(msgFromId) || msgFromId.equals(GlobalParams.loginUserId)) {
